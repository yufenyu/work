...
After that, Phoenix is heavily optimized by Yoo et al\cite{yoo2009phoenix2} to obtain better scalability on shared-memory system (\ie Phoenix2).
...
However, we note that the optimized Phoenix \cite{yoo2009phoenix2} (i.e., Phoenix2, which is named Phoenix in this paper) scales worse on a 32-core Intel 4× Xeon E7-4820 system running Ubuntu 12.04 with kernel 3.2.14.
...
Since the Phoenix runtime implements multithreading based on shared-memory Pthreads, all threads of the runtime have to share a single address space, which will lead to contention on the single lock \cite{clements2012concurrent}.
...
\begin{itemize}
  ...
  \item We develop a scalable thread library, \myth, in which threads run in  the separated memory space to avoid the contention on the shared space. Furthermore, \myth provides the \codet{unboundary-channel} for the threads to communicate with each others. 
