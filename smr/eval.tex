...
We compare the execution time of \myds with \code{Phoenix-ptmalloc} and \codet{Phoenix-jemalloc}, respectively.
...
\end{figure}
Figure \ref{fig:smr:time:jemalloc} shows the results of \myds comparing with \codet{Phoenix-jemalloc}. 
...
In addition, \myds spend more time then Phoenix when run \codet{linear\_regression} (Figrure \ref{fig:smr:time:ptmalloc}). 
...
Moreover, we find that initialized time is small variation for different benchmarks and in different cores count, except for \codet{pca}, from the experimental results (Figure \ref{fig:env:init}(b)).
...
Figure \ref{fig:dmr:hist:init} shows the result with \codet{exec} defined as mapreduce actual execution time and \codet{init} as the initialization time. 
...
However, the initialization time is almost constant, which cause the \codet{init} time ratio increase and dominate the total execution time at high cores count. 
