...
when the system exceeds its scalability limitation, 
...
All these pagefaults need to access the unique mmap\_sem semaphore, which is local to a process address space. As a result, multiple threads concurrently pagefaults will casue contention to the single semaphore.
...
For example, in \cite{Clements2012Scalable},  a scalable operation system is proposed by using ....;
...
Therefore, a key problem to be solved is that how to enable threads to eliminate contention on the per-process read/write semaphore when multiple threads concurrently run pagefaults.
