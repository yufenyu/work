...
\redt{
\_\_ticket\_spin\_lock is a type of spinlock which is caused by lock contention in Linux kernel.
In order to specially explore the impact of spinlock, we collect the execution time percent of \_\_ticket\_spin\_lock on each benchmark from 1 to 32 cores by Linux perf.
}
...
\redt{\_\_ticket\_spin\_lock is a type of spinlock which is caused by lock contention in Linux kernel.
	In Linux 3.2 kernel, \_\_ticket\_spin\_lock is used to ....
...
\redt{For example, in \cite{Clements2012Scalable},  a scalable operation system is proposed by using ....;
...
To address the problem and achieve our goal, we propose a novel thread programming model \myth (Scalable thread) with better scalability, supporting scalable MapReduce (\myds) compatibly, will be presented in this subsection.
...
In the case of \myds, at initial stage, the master thread invokes \codet{thread\_alloc} to allocate map threads and reduce threads, and then creates the shared-channel  between each pair of map  and reduce threads by invoking \codet{chan\_alloc}.
...
The master invokes \codet{chan\_setprod} and \codet{chan\_setcons} to set the map and reduce threads as producers and consumers for the shared-channel, respectively.
...
In our design, the shared-channel is a virtual memory area, called \code{CHAN} in the producer and consumer address space.
When the producer invokes \code{chan\_send} to send data, the sent data will be copyed to the \code{CHAN} area.
And then the consumer reads the data in the \code{CHAN} area  by invoking \code{chan\_recv}.
...
%extension
Initially, as shown in Figure \ref{fig:spmckern:extend}(a), the runtime will not allocate the actual page frames for \code{CHAN} area but map it to a special page frame--- anchor page table (\codet{Anchor1}) which is shared by the producer and consumer.
...
After that, by the anchor page (\code{Anchor1}), the consumer can locate the page frame \code{page1}  and then read data from \code{page1}.
...
To avoid the producer waiting, we design a channel buffer with unbounded size  by exploiting an \codet{extend} mechanism\cite{}.
This mechanism allows the producer sending data with no need for waiting when the channel buffer is full.
This \codet{extend} mechanism (Figure  \ref{fig:spmckern:extend}(b)) will remap the channel buffer (\code{CHAN} area) to another anchor and allocate some new page frames for the producer. 
And the consumer will not be disturbed by the \codet{extend} mechanism and it can continuously read the data from the original page frames.
