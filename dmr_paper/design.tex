\section{Design Of A Novel Thread Model}
\label{sec:design}
...
So, we give a \codet{shared-channel} for the threads to communicate with the others in \myth. 
...
In the case of \myds, at initial stage, the master thread invokes \codet{thread\_alloc} to allocate map threads and reduce threads, and then creates the \codet{shared-channel}  between each pair of map and reduce threads by invoking \codet{chan\_alloc}.
...
The producer sends messages to the \codet{shared-channel}, and the consumer receives the messages from it,
...
In our design, the \codet{shared-channel} is a virtual memory area, called \code{CHAN} in the producer and consumer address space.
...
In conclusion, we design a scalable thread model (\myth) and provide an unbounded \codet{shared-channel} for threads to communicate. This unbounded \codet{shared-channel} is a prominent feature of \myth, which is the main difference between our \myth and the previous works. 
