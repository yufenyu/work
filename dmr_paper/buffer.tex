...
%Phoenix中的buffer是一个全局二位数组，这个buffer被多个map worker和reduce worker所共享。为了避免共享buffer的barrier，DMR中我们采用私有的buffer，即每个map worker都拥有自己私有的buffer pool，这可以让Map 和 Reduce阶段并发执行。map worker的buffer pool中有多个buffer，每个buffer用于存放相应reduce worker的数据。当buffer中的数据达到一定的阈值时，便会触发一个send。
