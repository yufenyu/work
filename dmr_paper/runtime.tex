...
In cluster, it is the network dominate the performance, but when on single multicore system the performance is dominated by the operations on the data structure that holds intermediate data.
Defaultly, buffer in \myds is a hash table (Figure \ref{fig:dmr:hash-buffer}(a)), in which each element is a array of pointers to key arrays sorted by key.
%\myds stores the key-value pairs in an array sorted by key.
If the hash table have enough entries, collisions will be rare and the key arrays will be short,
so that lookup and insert will have cost O(1), which is an attractive quality for workloads.
...
Our producer-consumer model based on \myth requires data sent to the channel should be a contiguous block of memory.
Since key array in the hash buffer is scattered, which implies that all of the key arrays should be gathered together before sending it to the channel.
We address this issue by copying scattered key-value pairs out of the hash buffer and inserting them into a new contiguous region (Figure \ref{fig:dmr:hash-buffer}(b)).
This extra copy is unfortunate and time-consuming.
Furthermore, hash buffer also requires frequent allocations and deallocations of memory for key and value along with the data structure creation and destruction.
...
To avoid the time-consuming gather in hash buffer, we also implement a easy array buffer, and map worker could store its output by appending  key-value pairs to array buffer.
The array buffer are initially sized to a default value and then reuse the buffer in the hole of Map phase.
After sending the It will indicate that the buffer is empty at the end of a sending, but will not free the memory until all map jobs have finished.
This avoids the costs of expensive memory allocation and deallocation, as well as the data structures construction.
Above all, unlike hash buffer, array buffer is a contiguous block of memory, it not need gather before sending data in the buffer to the channel.
The experiment results show that the array buffer is more effective than the hash buffer for applications that likely have abundant key-value, such as word\_count (Section 5).
