...
Compared with existing work, \myds takes two main strategies to improve its scalability and performance.
...
In addition, the \code{Combiner} operation can reduce the communication traffic for the channel between map and reducer workers in \myds.
...
MRPhi adopts a typical producer-consumer model for pipelining, in which there is a many-to-one queue for communication between the map and reduce worker. 
% model is classically implemented as a common queue shared between one or more producers and one or more consumers
...
%There are some deficiencies of producer and consumer model in MRPhi.
%Since , 
...
%With the use of the one-to-one \code{shared-channel},
The one-to-one \code{shared-channel} means that the map worker sends the data to the reduce worker without contenting with other map workers.
Furthermore, as depicted in Section 3.3, the \code{shared-channel} is a no-boundary channel.
So, when the \code{shared-channel} buffer is full, the map worker can continuously work with no need for waiting the reduce worker to remove the data from the \code{shared-channel}.
Moreover, since the \code{shared-channel} is implementation by mapping, it can save overhead caused by dynamic memory allocation.
%Moreover, the map worker will set the \code{buffer} as empty after sending buffer's data.
%And the buffer will be reused in the whole Map phase, which saves overhead caused by dynamic memory allocation.
%When the \code{buffer} threshold is reached,
%the map worker will send data in \code{buffer} to the \code{shared-channel} and then set the \code{buffer} as empty.
