...
Since there are many heap objects shared among threads in Phoenix, and it is sensitive to memory allocator\cite{yoo2009phoenix2}.
...
Since Phoenix is sensitive to memory allocator\cite{yoo2009phoenix2}, in this subsection, we firstly evaluate its scalability with jemalloc, a good scalable memory allocator, and than we measure the scalability of \myds to demonstrate the effectivity of \myds .
...
We note that although Phoenix performs well with less cores, it can not scale up 16 cores for all applications. 
