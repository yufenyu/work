...
\end{figure}
{\bf Performance}
Based on \myth, We implemented the optimization of Phoenix in Section 4 and measured \myds performance by executing each benchmark, including histogram (\codet{hist}), linear\_regression (\codet{lr}), string\_match (\codet{sm}), word\_count (\codet{wc}), and pca.
